...
fn read_level_metadata(files: &FileManager, path: &Path) -> Result<WorldMetadata> {
    ...
    let mut decoder = GzDecoder::new(file).take(MAX_DECOMPRESSED_LEVEL_BYTES + 1);
...
fn game_mode(value: Option<i32>) -> String {
    match value {
        Some(0) => "survival",
        Some(1) => "creative",
        Some(2) => "adventure",
...
fn world_from_dir(files: &FileManager, world_dir: PathBuf) -> Option<WorldSummary> {
